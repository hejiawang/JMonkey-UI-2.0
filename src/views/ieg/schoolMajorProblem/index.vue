<template>
  <Layout v-layoutIn class="ieg-school-major-problem">
    <Row :gutter="32">
      <Col span="20"><Alert show-icon>【 {{schoolName}} 】 【{{majorName}}】 专业常见问题维护</Alert></Col>
      <Col span="2">
    ...
    <Row class="ieg-school-major-problem-main">
      <Row v-if="problemList.length === 0" class="ieg-school-major-problem-none">暂无问题</Row>
      <Collapse accordion v-else>
        <Panel v-for="problem in problemList" :name="problem.id" :key="problem.id">
          {{problem.problem}}
          <span class="ieg-school-major-problem-event"><Icon type="ios-trash-outline" @click="deleteHandle(problem)"/></span>
          <span class="ieg-school-major-problem-event"><Icon type="ios-brush-outline" @click="modifyHandle(problem)"/></span>
          <p slot="content">
    ...
    <CIegSchoolMajorProblemForm v-model="showForm" :type="formType" :schoolMajorId="majorId"
                           :problem="currentProblem" @refresh="initList"/>
  </Layout>
</template>
<script>
import CIegSchoolMajorProblemForm from '@/views/ieg/schoolMajorProblem/form'
import { list, del } from '@/api/ieg/schoolMajorProblem'
import 'quill/dist/quill.core.css'
import 'quill/dist/quill.snow.css'
import 'quill/dist/quill.bubble.css'

export default {
  name: 'IegSchoolMajorProblem',
  components: {
    CIegSchoolMajorProblemForm
  },
  computed: {
    /**
     * 当前院校名称
...
<style lang="scss">
  .ieg-school-major-problem {
    .ieg-school-major-problem-main {
      margin-top: 10px;
      ...
      .ieg-school-major-problem-none {
        font-size: 17px;
        line-height: 200px;
        text-align: center;
      }
      .ieg-school-major-problem-event {
        float: right;
        margin-right: 20px;
      }
    }
