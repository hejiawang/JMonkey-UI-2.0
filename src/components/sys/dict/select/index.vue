<template>
  <Select v-model="dictValue" :multiple="multiple" v-if="type == 'select'">
    <Option v-for="item in dictList" :value="item.value" :key="item.value">{{ item.lable }}</Option>
  </Select>
  <RadioGroup v-model="dictValue" v-else-if="type == 'radio' && !multiple">
    <Radio v-for="item in dictList" :label="item.value" :key="item.value">{{ item.lable }}</Radio>
  </RadioGroup>
  <CheckboxGroup v-model="dictValue" v-else>
    <Checkbox v-for="item in dictList" :label="item.value" :key="item.value">{{ item.lable }}</Checkbox>
  </CheckboxGroup>
</template>
<script>
